# AUTOGENERATED! DO NOT EDIT! File to edit: Renewables_Detector.ipynb.

# %% auto 0
__all__ = ['name', 'model', 'tokenizer', 's1_pipe', 'model_name', 'tokenizer_name', 's2_pipe', 'app', 'ack', 'x', 'txt_intro',
           'pdf_to_sentences', 'classify', 'setup_about_page', 'update_run_count']

# %% Renewables_Detector.ipynb 0
...
# %% Renewables_Detector.ipynb 2
# create pipeline for stage-1 classifier
name = "ESGBERT/EnvironmentalBERT-environmental"
model = AutoModelForSequenceClassification.from_pretrained(name)
tokenizer = AutoTokenizer.from_pretrained(name, max_len=512)
s1_pipe = pipeline("text-classification", model=model, tokenizer=tokenizer)

# %% Renewables_Detector.ipynb 3
# create pipeline for stage-2 classifier
model_name = "climatebert/renewable"
tokenizer_name = "climatebert/distilroberta-base-climate-detector"
model = AutoModelForSequenceClassification.from_pretrained(model_name)
tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, max_len=512)
s2_pipe = pipeline("text-classification", model=model, tokenizer=tokenizer)

# %% Renewables_Detector.ipynb 4
...
def classify(path, max_char=1000000):
    ...
    s1_class = s1_pipe(sents, padding=True, truncation=True)
    ...
    s2_class = s2_pipe(sents_s1, padding=True, truncation=True)
    ...
    df.loc[df['label'] == 'LABEL_1', 'label'] = 'Renewable'
    df.loc[df['label'] == 'LABEL_0', 'label'] = 'None'
    
    placeholder.empty()
    
    return(df)

# %% Renewables_Detector.ipynb 6
...
def setup_about_page(file, img1, img2):
    ...
    txt_demo = '''👋Welcome to Scetti's Renewables Detector tool!
    ...
    ack.markdown(ack_txt[5]) # climatebert/renewable acknowledgement
    
    # contact info
    txt_fb = '''📧Tell us about your experience with the Renewables Detector! Please send your 
    feedback and suggestions to <a href='mailto:ambar@scetti.org'> ambar@scetti.org</a>'''
    ack.markdown(txt_fb, unsafe_allow_html=True)

# %% Renewables_Detector.ipynb 7
...
# %% Renewables_Detector.ipynb 8
st.title("Renewables Detector")
...
txt_intro = '''This app uses AI-based Natural Language Processing to detect references to Renewable Energy and EVs 
in your climate policy document or corporate sustainability report.'''
...
if uploaded_file and submitted:
    ...
    if df is not None:
        ...
        app.subheader("Text containing references to Renewables", divider="blue")
